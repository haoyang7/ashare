package com.haoyoung.ashare.common.security.service;
 ...
 * @date 2021/12/21
 */
public interface AshareUserDetailsService extends UserDetailsService, Ordered {
	...
	default UserDetails getUserDetails(R<UserInfo> result) {
		...
		// 构造security用户
		return new AshareUser(user.getUserId(), user.getDeptId(), user.getUsername(),
	...
	/**
	 * 通过用户实体查询
	 * @param ashareUser user
	 * @return
	 */
	default UserDetails loadUserByUser(AshareUser ashareUser) {
		return this.loadUserByUsername(ashareUser.getUsername());
	}
