...
package com.haoyoung.ashare.common.xss.core;

import cn.hutool.core.util.CharsetUtil;
import com.haoyoung.ashare.common.xss.config.AshareXssProperties;
import com.haoyoung.ashare.common.xss.utils.XssUtil;
...
public class DefaultXssCleaner implements XssCleaner {
	...
	private final AshareXssProperties properties;

	public DefaultXssCleaner(AshareXssProperties properties) {
		this.properties = properties;
	}

	private static Document.OutputSettings getOutputSettings(AshareXssProperties properties) {
	...
	public String clean(String bodyHtml, XssType type) {
		...
		AshareXssProperties.Mode mode = properties.getMode();
		if (AshareXssProperties.Mode.escape == mode) {
			// html 转义
			return HtmlUtils.htmlEscape(bodyHtml, CharsetUtil.UTF_8);
		}
		else if (AshareXssProperties.Mode.validate == mode) {
			// 校验
